    ...
    def defineword(self, speech, language, regMatched):
        ...
        if len(output) == 1:
            ...
            if u'text' in answer:
                ...
                if worddef:
                    self.say(worddef, "Definisi dari {0} adalah: {1}".format(Question, worddef))
                else:
                    self.say("Maaf, saya tidak dapat menemukan " + Question + " di kamus.")
        else:
            self.say("Maaf, saya tidak dapat menemukan " + Question + " di kamus.")
